...
function eifnet(n, nstep, k, j0, ratewnt, τ, seedic, seednet)
    iext = τ * √k * j0 * ratewnt # iext given by balance equation
    ω = 2 * √iext # phase velocity EIF
    c = -j0 / √k * 2 / ω
    ϕth, ϕshift = π, 0.0# threshold for EIF
...
function ptc!(ϕ, postid, ϕshift, ω, c) # phase transition curve of EIF 
    for i in postid
...
@time eifnet(100, 1, 10, j0, ratewnt, τ, seedic, seednet);

# run & benchmark network with specified parameters
GC.gc()
@time rate, sidx, stimes = eifnet(n, nstep, k, j0, ratewnt, τ, seedic, seednet);
